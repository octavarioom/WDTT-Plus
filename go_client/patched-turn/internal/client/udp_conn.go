...
const (
	...
	// PATCH (WDTT mobile): was 30s. Shortened so failed channel binds are
	// re-attempted well within the ~20s DTLS handshake window, giving the
	// return path a chance to move onto ChannelData on restrictive links.
	defaultBindingCheckInterval = 2 * time.Second
	maxRetryAttempts              = 3
)
...
func (c *UDPConn) maybeBind(bound *binding) {
	...
	case state == bindingStateFailed:
		// PATCH (WDTT mobile): retry channel binds that previously failed.
		// On lossy / white-listed mobile links the first ChannelBind exchange is
		// often dropped; upstream then marks the binding Failed and never retries,
		// pinning the return path on STUN Data indications (which such networks
		// tend to drop). Re-arming the binding lets a later attempt succeed so
		// peer->client traffic can move onto ChannelData.
		bound.setState(bindingStateRequest)
